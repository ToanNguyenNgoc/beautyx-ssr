...
export function SerProItem(props: SerProItemProps) {
  ...
  const name = item.service_name ?? item.product_name ?? item.name
  ...
  if (type === "COMBO") LINK_DETAIL = formatRouterLinkCombo(id, org_dis.id, name)
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span className={!changeStyle ? style.item_name : `${style.item_name} ${style.item_name_ch}`}
          >{name}</span>
          {
            type === 'COMBO' ?
              <div className={!changeStyle ? style.item_price : style.item_price_ch}>
                {
                  item.special_price_momo > 0 ?
                    <>
                      <span>{formatPrice(item.special_price_momo)}</span>
                      <span>{formatPrice(item.use_value)}</span>
                    </>
                    :
                    <span style={{ color: "var(--purple)" }} >{formatPrice(item.use_value)}</span>
                }
              </div>
              :
              <div className={!changeStyle ? style.item_price : style.item_price_ch}>
